...
//	fprintf(xsl,"		</xsl:if>\r\n");
	...
	printf("{\n");
		printf("xml = loadXMLDoc(\"%s\");\n",WEB_PATH_FILE_DOENTES_NOVOS_PACIENTE_ADICIONADO_XML);
		printf("xsl = loadXMLDoc(\"%s\");\n",WEB_PATH_FILE_DOENTES_NOVOS_VER_PACIENTE_XSL);
		printf("\n");
		// code for IE
		printf("if (window.ActiveXObject)\n");
		printf("{\n");
	...
	printf("</script>\n");
	printf("</head>\n");
	printf("<body bgcolor=\"white\" id=\"result\" onLoad=\"displayResult(), window.print()\">\n");
	printf("</body>\n");
	printf("</html>");	
	exit(OK);
}
