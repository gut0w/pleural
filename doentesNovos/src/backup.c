...
//	time(&seg);
//	tempo=localtime(&seg);LOCAL_PATH_FILE_MORRYS_PACIENTE_ADICIONADO_XML
...
//	snprintf (date,DATE_LENGTH+1,"%04i-%02i-%02i_%02i-%02i-%02i",1900+tempo->tm_year,tempo->tm_mon+1,tempo->tm_mday,tempo->tm_hour,tempo->tm_min,tempo->tm_sec);
	...
	strcat(fileName,"~/public_html/dev/backup/");
...
//	}
	...
	printf("Content-Disposition: attachment; filename=pacienteMorrys_%s_backup.xml\r\n\r\n",date);	
/*	fgets(temp, 1024, xml);
//	printf("%s\n",temp);
	while (!feof(xml))
	{
